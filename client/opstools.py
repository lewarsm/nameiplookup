   ...
   #     self.start_https_server()
    ...
    def open_oidc_log_window(self):
        if self.oidc_log_window is None or not self.oidc_log_window.winfo_exists():
            ...
            self.oidc_log_text = tk.Text(self.oidc_log_window)
...
class HTTPRequest:
    ...
    def setup_ui(self):
        ...
        self.use_custom_hosts_button = ttk.Button(self.frame, text="Use Custom Hosts File", command=self.use_custom_hosts)
        self.use_custom_hosts_button.grid(row=1, column=6, padx=5, pady=5)  # Adjust row=XX as needed
    ...
    def use_custom_hosts(self):
        try:
            with open('hosts.json', 'r') as file:
                hosts_content = json.load(file)
            # Logic to use hosts_content for DNS override
            tk.messagebox.showinfo("Info", "Using custom hosts file for DNS.")
        except FileNotFoundError:
            tk.messagebox.showerror("Error", "Hosts file not found.")
