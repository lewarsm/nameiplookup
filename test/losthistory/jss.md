...
class OIDCDebugger:
    ...
    def setup_ui(self):
        ...
        self.replace_certificate_btn.grid(row=10, column=0, padx=5, pady=5)
    ...
    def display_well_known_response(self, config):
        for widget in self.response_table_frame.winfo_children():
            widget.destroy()
        columns = ("Key", "Value")
        table = ttk.Treeview(self.response_table_frame, columns=columns, show="headings")
