...
class JWKSCheck:
     def __init__(self, master, style):
         ...
         # PingOne self.default_url = "https://auth.pingone.com/0a7af83d-4ed9-4510-93cd-506fe835f69a/as/jwks"
             ...
             for key in jwks.get('keys', []):
                 if 'x5c' in key:
                     for cert in key['x5c']:
                         ...
                         not_valid_before = x509_cert.not_valid_before_utc.strftime('%Y-%m-%d %H:%M:%S')
                         not_valid_after = x509_cert.not_valid_after_utc.strftime('%Y-%m-%d %H:%M:%S')
